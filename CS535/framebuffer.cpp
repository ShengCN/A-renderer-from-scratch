...
bool FrameBuffer::LoadTextureGPU(const std::string texFile)
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, &newTex->texture[0]);
...
bool FrameBuffer::LoadCubemapGPU(vector<string> cubemaps)
{
	GLuint textureID;
...
float FrameBuffer::ColorDifference(shared_ptr<PPC> ppc0, shared_ptr<FrameBuffer> fb1, shared_ptr<PPC> ppc1)
{
	...
	for(int v = 0; v < h; ++v)
	{
		for(int u = 0; u < w; ++u)
		{
			...
			// Difference vector distance (squared)
			ret += (c1v - c0v) * (c1v - c0v);
			opn++;
		}
	}
